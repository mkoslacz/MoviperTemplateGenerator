...
<globals>
	...
	<global
		id="screenBaseName"
		value="Viper<#if screenType?contains("Ai")><#if viewState>ViewState</#if>${screenType?replace("Passive", "")}<#if screenType?contains("Passive")>Passive</#if>Fragment<#else>${screenType?replace("Passive", "")}<#if viewState>ViewState</#if><#if screenType?contains("Passive")>Passive</#if>Fragment</#if>"/>
</globals>
